...
class DotProdNB(BaseNet):
    def __init__(self, vocab_size, n_classes, r, w_adj=0.4, r_adj=10):
        ...
        self.r.weight.data = torch.Tensor(np.concatenate([np.zeros((1, n_classes)), r])).to(device)

        np.savetxt('test.out', self.r.weight.data, delimiter=',')
        np.savetxt("r.out",self.r.weight.data.view(1, -1), fmt='%.06f', delimiter=',')
        np.savetxt("w.out", self.w.weight.data.view(1, -1), fmt='%.06f', delimiter=',')
    ...
    def forward(self, feat_idx):
        ...
        x = x / self.r_adj
        np.savetxt("feat_idx.out", feat_idx.data.view(1, -1), fmt='%d', delimiter=',')
        np.savetxt("x.out", x.data.view(1, -1), fmt='%.06f', delimiter=',')
        return x
...
model = DotProdNB(args.vocab_size, n_classes, r).to(device)
model.verbose = True
